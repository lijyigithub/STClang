...
class _CXString(Structure):
    ...
    def __repr__(self):
        if self.spelling is not None:
            return self.spelling.decode("utf-8", errors="ignore")
        else:
            return ""
...
class Diagnostic(object):
    ...
    @property
    def spelling(self):
        sp = conf.lib.clang_getDiagnosticSpelling(self)
        if isinstance(sp, bytes):
            return sp.decode("utf-8", errors="ignore")
        else:
            return sp
...
class Cursor(Structure):
    ...
    def get_usr(self):
        ...
        usr = conf.lib.clang_getCursorUSR(self)
        if isinstance(usr, bytes):
            return usr.decode('utf-8', errors='ignore')
        else:
            return usr
    ...
    def kind(self):
        ...
        kd =  CursorKind.from_id(self._kind_id)
        if isinstance(kd, bytes):
            return kd.decode("utf-8", errors="ignore")
        else:
            return kd
    ...
    def brief_comment(self):
        ...
        cmt = conf.lib.clang_Cursor_getBriefCommentText(self)
        if isinstance(cmt, bytes):
            return cmt.decode("utf-8", errors="ignore")
        else:
            return cmt
...
class TypeKind(BaseEnumeration):
    ...
    def spelling(self):
        ...
        sp = conf.lib.clang_getTypeKindSpelling(self.value)
        if isinstance(sp, bytes):
            return sp.decode("utf-8", errors="ignore")
        else:
            return sp
...
class Type(Structure):
    ...
    def spelling(self):
        ...
        sp = conf.lib.clang_getTypeSpelling(self)
        if isinstance(sp, bytes):
            return sp.decode("utf-8", errors="ignore")
        else:
            return sp
...
class CompletionChunk:
    ...
    def spelling(self):
        if self.__kindNumber in SpellingCache:
            sp = SpellingCache[self.__kindNumber]
            if isinstance(sp, bytes):
                return sp.decode("utf-8", errors="ignore")
            else:
                return sp
        sp = conf.lib.clang_getCompletionChunkText(self.cs, self.key).spelling
        if isinstance(sp, bytes):
            return sp.decode("utf-8", errors="ignore")
        else:
            return sp
...
class TranslationUnit(ClangObject):
    ...
    @classmethod
    def create_from_source(cls, filename, args=None, unsaved_files=None):
        if args is None:
            args = []
        ...
        if not filename is None:
            filename = filename.encode("utf-8")
        ptr = conf.lib.clang_createTranslationUnitFromSourceFile(index, filename, len(args),
                                     args_array, 
                                     len(unsaved_files),
                                     unsaved_array)
    ...
    def spelling(self):
        ...
        sp = conf.lib.clang_getTranslationUnitSpelling(self)
        if isinstance(sp, bytes):
            return sp.decode("utf-8", errors="ignore")
        else:
            return sp
...
class File(ClangObject):
    ...
    def name(self):
        ...
        nm =  conf.lib.clang_getCString(conf.lib.clang_getFileName(self))
        if isinstance(nm, bytes):
            return nm.decode("utf-8", errors="ignore")
        else:
            return nm
...
class Token(Structure):
    ...
    def spelling(self):
        ...
        """
        sp = conf.lib.clang_getTokenSpelling(self._tu, self)
        if isinstance(sp, bytes):
            return sp.decode("utf-8", errors="ignore")
        else:
            return sp
...
#  CXTUResourceUsage),
  ...
  ("clang_createTranslationUnitFromSourceFile",
   [Index, c_char_p, c_int, c_void_p, c_int, c_void_p],
   c_object_p),
